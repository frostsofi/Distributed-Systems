...
public class Main extends ComponentDefinition {
	...
	public Main() {
		...
		//Создание всех компонентов программы
		...
		//Подписываем порт по умолчанию на обработку ошибок
		...
		//связь консоли и приложения, отправка команд от консоли в приложение
		connect(app.required(Console.class), console.provided(Console.class));

		//связь приложения и perfect link, это сязь между портом pp2p  и app, в кострукторах порты
		//приложение получает сообщение и выводит в консоль, а перфект линк его генерирует
		connect(app.required(PerfectPointToPointLink.class), pp2p.provided(PerfectPointToPointLink.class));

		//приложение получает сообщение и выводит в консоль, а фэйр-лос линк его генерирует
		connect(app.required(FairLossPointToPointLink.class), flp2p.provided(FairLossPointToPointLink.class));

		//просыпание из задержки
